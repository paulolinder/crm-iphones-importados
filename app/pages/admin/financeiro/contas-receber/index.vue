...
import type { AccountsReceivable } from '~~/domains/financeiro/types'

definePageMeta({ layout: 'admin' })
useHead({ title: 'Contas a receber' })

const { listReceivables } = useFinance()
const { format } = useCurrency()

const loading = ref(true)
const err = ref<string | null>(null)
const rows = ref<AccountsReceivable[]>([])
const total = ref(0)
const page = ref(1)
const perPage = 20
...
const load = async () => {
  ...
  err.value = null
  try {
    const res = await listReceivables({ page: page.value, per_page: perPage })
    rows.value = res.data
...
<template>
  <div class="p-4 lg:p-8 space-y-6">
    <AppPageHeader
      ...
      description="Tabela accounts_receivable"
      :breadcrumbs="[
        { label: 'Financeiro', to: '/admin/financeiro' },
        { label: 'Contas a receber' },
      ]"
      :actions="[
        { key: 'new', label: 'Novo recebível', icon: 'lucide:plus', variant: 'primary', to: '/admin/financeiro/contas-receber/nova' },
      ]"
    />
